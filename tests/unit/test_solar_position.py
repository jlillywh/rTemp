...
class TestSunriseSunset:
    ...
    def test_photoperiod_longer_in_summer(self):
        ...
        # Summer solstice
        sunrise_summer = NOAASolarPosition.calc_sunrise(lat, lon, 2020, 6, 21, timezone, dlstime_summer)
        sunset_summer = NOAASolarPosition.calc_sunset(lat, lon, 2020, 6, 21, timezone, dlstime_summer)
        photoperiod_summer = (sunset_summer - sunrise_summer) * 24
        
        # Winter solstice
        sunrise_winter = NOAASolarPosition.calc_sunrise(lat, lon, 2020, 12, 21, timezone, dlstime_winter)
        sunset_winter = NOAASolarPosition.calc_sunset(lat, lon, 2020, 12, 21, timezone, dlstime_winter)
        photoperiod_winter = (sunset_winter - sunrise_winter) * 24
        
        assert photoperiod_summer > photoperiod_winter, \
...
class TestHelperFunctions:
    ...
    def test_sun_rad_vector_range(self):
        ...
        for month in range(1, 13):
            ...
            assert 0.98 < distance < 1.02, \
                f"Distance for month {month} out of range: {distance}"
    ...
    def test_equation_of_time_range(self):
        ...
        for month in range(1, 13):
            ...
            assert -20 < eq_time < 20, \
                f"Equation of time for month {month} out of range: {eq_time}"
    ...
    def test_declination_range(self):
        ...
        for month in range(1, 13):
            ...
            assert -24 < decl < 24, \
                f"Declination for month {month} out of range: {decl}"
