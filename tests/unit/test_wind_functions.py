...
class TestVirtualTemperatureDifference:
    ...
    def test_vapor_pressure_effect(self):
        ...
        delta_t_v1 = calculate_virtual_temperature_difference(air_temp, water_temp, 10.0, 10.0)

        # Case 2: Different vapor pressures
        delta_t_v2 = calculate_virtual_temperature_difference(air_temp, water_temp, 10.0, 15.0)
...
class TestWindFunctionComparison:
    ...
    def test_all_methods_increase_with_wind(self):
        ...
        for method in methods:
            result_low = method.calculate(
                2.0, air_temp, water_temp, vapor_pressure_air, vapor_pressure_water
            )
            result_high = method.calculate(
                8.0, air_temp, water_temp, vapor_pressure_air, vapor_pressure_water
            )

            assert (
                result_high > result_low
            ), f"{method.__class__.__name__} did not increase with wind speed"
