...
def main():
    ...
    vba_output_24hr = vba_output[
        (vba_output['datetime'] >= start_time) & 
        (vba_output['datetime'] < end_time)
    ].copy()
    
    vba_input_24hr = vba_input[
        (vba_input['datetime'] >= start_time) & 
        (vba_input['datetime'] < end_time)
    ].copy()
    ...
    met_data = vba_input_24hr[['datetime', 'air temperature (deg C)', 
                                 'dewpoint temperature (deg C)', 
                                 'wind speed (m/s)', 
                                 'cloud cover (fraction)',
                                 'shortwave solar radiation before shade and reflection (W/m^2)']].copy()
    met_data.columns = ['datetime', 'air_temperature', 'dewpoint_temperature', 
                        'wind_speed', 'cloud_cover', 'solar_radiation']
    ...
    vba_output_24hr = vba_output_24hr.rename(columns={
    ...
    for field, label in comparison_fields:
        ...
        print(f"{label}:")
        print(f"  VBA:    mean={np.mean(vba_vals):8.2f}  min={np.min(vba_vals):8.2f}  max={np.max(vba_vals):8.2f}")
        print(f"  Python: mean={np.mean(python_vals):8.2f}  min={np.min(python_vals):8.2f}  max={np.max(python_vals):8.2f}")
        print(f"  Diff:   mean={np.mean(diff):8.2f}  max_abs={np.max(abs_diff):8.2f}  RMSE={np.sqrt(np.mean(diff**2)):8.2f}")
        print()
    ...
    for idx in key_times:
        ...
        for field, label in comparison_fields:
            if field == 'potential_solar':
                ...
                print(f"  {label:30s}  VBA: {vba_val:10.2f}  Python: {python_val:10.2f}  Diff: {diff:10.2f}")
        print()
    ...
    print("-" * 80)
    for s in stats:
        print(f"{s['field']:<30} {s['vba_mean']:10.2f} {s['python_mean']:10.2f} "
