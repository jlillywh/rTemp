...
def investigate_timestep(dt, vba_data, met_data, config_params):
    ...
    print(
        f"  Difference from VBA: {potential_solar - vba_potential:.2f} W/m² ({(potential_solar - vba_potential) / vba_potential * 100:.1f}%)"
    )
    print()
    ...
    solar_after_shade = SolarRadiationCorrections.apply_shade_correction(
        solar_after_cloud, effective_shade=config_params["effective_shade"]
    )
    ...
    print(
        f"  Difference:   {solar_final - vba_final:.2f} W/m² ({(solar_final - vba_final) / vba_final * 100 if vba_final > 0 else 0:.1f}%)"
    )
    print()
...
def main():
    ...
    for idx, row in vba_daytime.iterrows():
        ...
        if elevation > 0:
            ...
            solar_temp = SolarRadiationCorrections.apply_cloud_correction(
                python_potential, cloud_cover, 0.65, 2.0
            )
