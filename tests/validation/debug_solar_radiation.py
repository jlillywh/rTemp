...
daytime_mask = vba_output['solar radiation entering the water after shade and reflection (W/m^2)'] > 100
...
for idx in daytime_indices[:3]:
    ...
    vba_solar = vba_output['solar radiation entering the water after shade and reflection (W/m^2)'].iloc[idx]
    vba_potential = vba_output['potential solar radiation at the ground surface before shade and reflection (W/m^2)'].iloc[idx]
    ...
    if elevation > 0:
        ...
        print(f"  Difference: {solar_final - vba_solar:.2f} W/m² ({(solar_final - vba_solar) / vba_solar * 100:.1f}%)")
    else:
...
vba_solar_all = vba_output['solar radiation entering the water after shade and reflection (W/m^2)'].values
...
print(f"Percent difference: {(np.mean(daytime_python) - np.mean(daytime_vba)) / np.mean(daytime_vba) * 100:.1f}%")
