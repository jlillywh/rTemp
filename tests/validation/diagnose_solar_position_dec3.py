...
# Hour angle
ha = NOAASolarPosition.calc_hour_angle(
    hour, config["longitude"], eq_time, config["timezone"], config["daylight_savings"]
)
...
for tz_offset in [-8.0, -7.0]:
    for dst in [0, 1]:
        ...
        print(
            f"  TZ={tz_offset:+.1f}, DST={dst}: Elevation={elev:.2f}° (diff from VBA: {elev - vba_noon['solar elevation (deg)']:+.2f}°)"
        )
