...
wind_speed_strategy = st.floats(min_value=0.0, max_value=50.0, allow_nan=False, allow_infinity=False)

# Strategy for generating valid temperatures (reasonable meteorological range)
temperature_strategy = st.floats(min_value=-10.0, max_value=45.0, allow_nan=False, allow_infinity=False)
...
def realistic_meteorological_conditions(draw):
    ...
    air_temp = draw(st.floats(min_value=-10.0, max_value=45.0, allow_nan=False, allow_infinity=False))
    water_temp = draw(st.floats(min_value=-10.0, max_value=45.0, allow_nan=False, allow_infinity=False))
    ...
    vapor_pressure_air = draw(st.floats(
    ...
    vapor_pressure_water = draw(st.floats(
...
class TestWindFunctionMonotonicityProperty:
    ...
    # Validates: Requirements 5.1-5.5
    @given(
        wind_speed_low=st.floats(min_value=0.0, max_value=25.0, allow_nan=False, allow_infinity=False),
        wind_speed_high=st.floats(min_value=25.0, max_value=50.0, allow_nan=False, allow_infinity=False),
        conditions=realistic_meteorological_conditions(),
    )
    @settings(max_examples=100)
    def test_brady_graves_geyer_monotonicity(
        ...
        result_low = wf.calculate(
            wind_speed_low, air_temp, water_temp,
            vapor_pressure_air, vapor_pressure_water
        )
        result_high = wf.calculate(
            wind_speed_high, air_temp, water_temp,
            vapor_pressure_air, vapor_pressure_water
        )
