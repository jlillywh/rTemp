    ...
    def calculate(
        ...
        **kwargs
        ...
        Returns:
            Atmospheric emissivity (0-1)
        """
        emissivity = 0.52 + 0.065 * math.sqrt(vapor_pressure_mmhg)
...
class EmissivityBrutsaert(LongwaveEmissivity):
    ...
    def calculate(
        ...
        **kwargs
        ...
        emissivity = self.coefficient * (vapor_pressure_mb / air_temp_k) ** (1.0 / 7.0)
        # Clamp to physically valid range [0, 1]
        return max(0.0, min(1.0, emissivity))
...
class EmissivitySatterlund(LongwaveEmissivity):
    ...
    def calculate(
        ...
        **kwargs
        ...
        emissivity = 1.08 * (1.0 - math.exp(-exponent))
        # Clamp to physically valid range [0, 1]
        return max(0.0, min(1.0, emissivity))
    ...
    def calculate(
        ...
        **kwargs
        ...
        Returns:
            Atmospheric emissivity (0-1)
        """
        emissivity = 1.0 - 0.261 * math.exp(-0.000777 * air_temp_c ** 2)
        # Clamp to physically valid range [0, 1]
        return max(0.0, min(1.0, emissivity))
...
class EmissivitySwinbank(LongwaveEmissivity):
    ...
    def calculate(
        ...
        **kwargs
        ...
        """
        air_temp_k = air_temp_c + CELSIUS_TO_KELVIN
        emissivity = 0.92e-5 * air_temp_k ** 2
        # Clamp to physically valid range [0, 1]
        return max(0.0, min(1.0, emissivity))
