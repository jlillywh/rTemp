...
class SolarRadiationRyanStolz:
    ...
    def calculate(
        ...
        clear_sky_radiation: float = radiation_toa * (atmospheric_transmission_coeff**relative_air_mass)

        return clear_sky_radiation

    @staticmethod
    def _calc_relative_air_mass(elevation: float, site_elevation_m: float) -> float:
        """
        Calculate relative air mass corrected for site elevation.
        ...
        air_mass_denominator: float = sin_elevation + 0.15 * ((elevation + 3.885) ** -1.253)

        relative_air_mass: float = pressure_ratio / air_mass_denominator
