...
class SolarRadiationBird:
    ...
    def calculate(
        ...
        ta = SolarRadiationBird._calc_aerosol_transmittance(
            aod_500nm, aod_380nm, air_mass
        )
        ...
        diffuse_rayleigh = (
            ...
            cos_zenith *
            to * tg * tw * ta *
            (1.0 - tr) / (1.0 - air_mass_pressure + air_mass_pressure ** 1.02)
        )
        ...
        diffuse_aerosol = (
            ...
            to * tg * tw *
            forward_scatter *
            (1.0 - taa) / (1.0 - air_mass_pressure + air_mass_pressure ** 1.02)
        )
        ...
        diffuse_multiple = (
            ...
            albedo * (1.0 - tr) / (1.0 - albedo * (1.0 - tr))
        )
    ...
    def _calc_rayleigh_transmittance(air_mass_pressure: float) -> float:
        ...
        tr = math.exp(-0.0903 * (air_mass_pressure ** 0.84) * (1.0 + air_mass_pressure - air_mass_pressure ** 1.01))
    ...
    def _calc_ozone_transmittance(ozone_cm: float, air_mass: float) -> float:
        ...
        to = 1.0 - 0.1611 * ozone_path * (1.0 + 139.48 * ozone_path) ** -0.3035 - 0.002715 * ozone_path / (1.0 + 0.044 * ozone_path + 0.0003 * ozone_path ** 2)
    ...
    def _calc_water_vapor_transmittance(water_cm: float, air_mass: float) -> float:
        ...
        tw = 1.0 - 2.4959 * water_path / ((1.0 + 79.034 * water_path) ** 0.6828 + 6.385 * water_path)
        
        return tw
    
    @staticmethod
    def _calc_aerosol_transmittance(
        aod_500nm: float,
        aod_380nm: float,
        air_mass: float
    ) -> float:
        ...
        ta = math.exp(-aod_broadband ** 0.873 * (1.0 + aod_broadband - aod_broadband ** 0.7088) * air_mass ** 0.9108)
