...
class SolarRadiationIqbal:
    ...
    def calculate(
        ...
        tau_aa = 1.0 - (1.0 - SolarRadiationIqbal.SINGLE_SCATTERING_ALBEDO) * \
                 (1.0 - ma + ma ** 1.06) * (1.0 - tau_a)
        
        # Rayleigh-scattered diffuse irradiance
        diffuse_rayleigh = (
            0.79 * rho2 * SOLAR_CONSTANT * cos_zenith *
            tau_o * tau_g * tau_w * tau_aa *
            0.5 * (1.0 - tau_r) / (1.0 - ma + ma ** 1.02)
        )
        ...
        diffuse_aerosol = (
            ...
            (1.0 - tau_as) / (1.0 - ma + ma ** 1.02)
        )
        
        # Multiple scattering between ground and atmosphere
        alpha_atmos = 0.0685 + (1.0 - SolarRadiationIqbal.FORWARD_SCATTER_RATIO) * \
                      (1.0 - tau_as)
        
        diffuse_multiple = (
            (direct_hz + diffuse_rayleigh + diffuse_aerosol) *
            albedo * alpha_atmos / (1.0 - albedo * alpha_atmos)
        )
    ...
    def _calc_relative_air_mass(elevation: float) -> float:
        ...
        mr = 1.0 / (math.cos(zenith * DEG_TO_RAD) + 
                    0.15 * ((93.885 - zenith) ** -1.253))
    ...
    def _calc_water_vapor_saturation(temperature_k: float) -> float:
        ...
        wvap_s = (a0 + temp_c * (a1 + temp_c * (a2 + temp_c * 
                 (a3 + temp_c * (a4 + temp_c * (a5 + temp_c * a6))))))
    ...
    def _calc_ozone_transmittance(ozone_cm: float, mr: float) -> float:
        ...
        tau_o = 1.0 - (
            ...
            0.002715 * ozone_path / 
            (1.0 + 0.044 * ozone_path + 0.0003 * ozone_path ** 2)
        )
