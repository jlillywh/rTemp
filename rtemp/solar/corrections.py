...
class SolarRadiationCorrections:
    ...
    def apply_cloud_correction(
        solar_radiation: float, cloud_cover: float, kcl1: float = 1.0, kcl2: float = 2.0
    ) -> float:
        """
        ...
        # Apply correction
        corrected_radiation: float = solar_radiation * correction_factor

        return max(0.0, corrected_radiation)

    @staticmethod
    def apply_shade_correction(solar_radiation: float, effective_shade: float) -> float:
        """
    ...
    @staticmethod
    def calculate_anderson_albedo(cloud_cover: float, solar_elevation: float) -> float:
        """
        ...
        albedo: float = albedo_clear * (1.0 - cloud_cover) + albedo_overcast * cloud_cover
    ...
    @staticmethod
    def apply_albedo_correction(solar_radiation: float, albedo: float) -> float:
        """
    ...
    def apply_all_corrections(
        ...
        # Apply shade correction
        after_shade = SolarRadiationCorrections.apply_shade_correction(after_cloud, effective_shade)
        ...
        if use_anderson_albedo:
            ...
            final = SolarRadiationCorrections.apply_albedo_correction(after_shade, albedo)
            result["final"] = final
        else:
            result["albedo"] = 0.0
