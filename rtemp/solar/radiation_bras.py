...
class SolarRadiationBras:
    ...
    @staticmethod
    def calculate(elevation: float, earth_sun_distance: float, turbidity: float = 2.0) -> float:
        """
        ...
        extraterrestrial_radiation = (SOLAR_CONSTANT / (earth_sun_distance**2)) * math.sin(
            elevation_rad
        )
        ...
        clear_sky_radiation = extraterrestrial_radiation * math.exp(
            -turbidity * scattering_coeff * air_mass
        )
    ...
    def _calc_optical_air_mass(elevation: float) -> float:
        ...
        elevation_rad = elevation * DEG_TO_RAD
        air_mass: float = 1.0 / (math.sin(elevation_rad) + 0.15 * ((elevation + 3.885) ** -1.253))
    ...
    def _calc_molecular_scattering_coeff(air_mass: float) -> float:
        ...
        scattering_coeff: float = 0.128 - 0.054 * math.log10(air_mass)
