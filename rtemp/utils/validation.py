...
class InputValidator:
    ...
    def validate_site_parameters(params: Dict[str, Any]) -> Tuple[Dict[str, Any], List[str]]:
        ...
        if "sediment_thermal_diffusivity" in params:
            if params["sediment_thermal_diffusivity"] <= 0:
                ...
                warnings.append(
                    "Sediment thermal diffusivity was zero or negative, assumed equal to water properties"
                )
    ...
    def validate_meteorological_data(data: pd.DataFrame) -> Tuple[pd.DataFrame, List[str]]:
        ...
        if "dewpoint_temperature" in validated.columns:
            ...
            if missing_dewpoint.any():
                ...
                warnings.append(
                    f"Dewpoint temperature missing for {count} timestep(s), set to 10°C"
                )
    ...
    def check_timestep(
        ...
        if timestep_days == 0:
            return (
                "Duplicate timestep detected (timestep = 0), skipping temperature update",
                timestep_days,
            )
        ...
        if timestep_days < 0:
            return (
                f"Timestep is negative ({timestep_days:.4f} days), timestamps may not be monotonically increasing",
                timestep_days,
            )
        ...
        if timestep_hours > 2.0:
            if timestep_hours > 4.0:
                ...
                return (
                    f"Large timestep detected ({timestep_hours:.2f} hours), consider resetting temperatures to midpoint of air and dewpoint",
                    timestep_days,
                )
            else:
                return (
                    f"Timestep greater than 2 hours ({timestep_hours:.2f} hours), potential accuracy issues",
                    timestep_days,
                )
