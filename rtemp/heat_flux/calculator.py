...
class HeatFluxCalculator:
    ...
    def calculate_evaporation(
        wind_function: float, vapor_pressure_water: float, vapor_pressure_air: float
    ) -> float:
        """
    ...
    def calculate_convection(
        wind_function: float, water_temp: float, air_temp: float, bowen_ratio: float = BOWEN_RATIO
    ) -> float:
        """
    ...
    def calculate_hyporheic_exchange(
        water_temp: float, sediment_temp: float, exchange_rate: float, water_depth: float
    ) -> float:
        """
        ...
        hyporheic_flux = density_g_cm3 * specific_heat_cal * exchange_rate * temp_diff / depth_cm

        return hyporheic_flux

    @staticmethod
    def calculate_groundwater_flux(
        water_temp: float, groundwater_temp: float, inflow_rate: float, water_depth: float
    ) -> float:
        """
        ...
        groundwater_flux = density_g_cm3 * specific_heat_cal * inflow_rate * temp_diff / depth_cm
